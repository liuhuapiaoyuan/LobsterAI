...
export class OpenClawRuntimeAdapter extends EventEmitter implements CoworkRuntime {
   ...
   * so YuanAI can recover even when the gateway fails to deliver the abort event.
   */
   ...
   * for a YuanAI-initiated session.
   */
  ...
  private buildSystemPromptPrefix(systemPrompt: string): string {
    return [
      '[YuanAI system instructions]',
      'Apply the instructions below as the highest-priority guidance for this session.',
      'If earlier YuanAI system instructions exist, replace them with this version.',
      systemPrompt,
    ].join('\n');
  }
  ...
  private buildBridgePrefix(messages: CoworkMessage[], currentPrompt: string): string {
    ...
    return [
      '[Context bridge from previous YuanAI conversation]',
   ...
   * gateway rather than the YuanAI UI.
   *
