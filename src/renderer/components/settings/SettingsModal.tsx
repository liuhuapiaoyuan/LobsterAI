...
import type { SettingsTab, CoworkSection, SettingsOpenOptions } from './settingsTypes';
import { resolveSettingsRoute } from './settingsTypes';
...
const SettingsModal: React.FC<SettingsProps> = ({ onClose, initialTab, initialCoworkSection, notice, onUpdateFound }) => {
  const dispatch = useDispatch();
  const [activeTab, setActiveTab] = useState<SettingsTab>(() =>
    resolveSettingsRoute({ initialTab: initialTab as string | undefined, initialCoworkSection }).tab,
  );
  const [coworkSection, setCoworkSection] = useState<CoworkSection>(() =>
    resolveSettingsRoute({ initialTab: initialTab as string | undefined, initialCoworkSection }).coworkSection,
  );
  ...
  useEffect(() => {
    const r = resolveSettingsRoute({ initialTab: initialTab as string | undefined, initialCoworkSection });
    setActiveTab(r.tab);
    setCoworkSection(r.coworkSection);
  }, [initialTab, initialCoworkSection]);
  ...
  useEffect(() => {
    if (activeTab !== 'cowork' || coworkSection !== 'memory') return;
    void loadCoworkMemoryData();
  }, [activeTab, coworkSection, loadCoworkMemoryData]);
  ...
  useEffect(() => {
    if (activeTab !== 'cowork' || coworkSection !== 'persona') return;
    if (!bootstrapLoaded) {
      void (async () => {
  ...
  }, [activeTab, coworkSection, bootstrapLoaded]);
  ...
  const sidebarTabs: { key: SettingsTab; label: string; icon: React.ReactNode }[] = useMemo(() => [
    { key: 'app', label: i18nService.t('settingsTabApp'), icon: <Cog6ToothIcon className="h-5 w-5" /> },
    { key: 'models', label: i18nService.t('settingsTabModels'), icon: <CubeIcon className="h-5 w-5" /> },
    { key: 'cowork', label: i18nService.t('settingsTabCowork'), icon: <CpuChipIcon className="h-5 w-5" /> },
    { key: 'integrations', label: i18nService.t('settingsTabIntegrations'), icon: <PuzzlePieceIcon className="h-5 w-5" /> },
    { key: 'about', label: i18nService.t('settingsTabAbout'), icon: <InformationCircleIcon className="h-5 w-5" /> },
  ], [language]);

  const coworkSectionLabels = useMemo(() => ({
    engine: i18nService.t('coworkSectionEngine'),
    memory: i18nService.t('coworkSectionMemory'),
    persona: i18nService.t('coworkSectionPersona'),
  }), [language]);

  const activeTabLabel = useMemo(() => {
    if (activeTab === 'cowork') {
      return `${i18nService.t('settingsTabCowork')} · ${coworkSectionLabels[coworkSection]}`;
    }
    return sidebarTabs.find(t => t.key === activeTab)?.label ?? '';
  }, [activeTab, coworkSection, coworkSectionLabels, sidebarTabs, language]);
        ...
        return (
          <div className="space-y-8">
            ...
            <SettingsToggleRow
              ...
              onChange={() => setUseSystemProxy((prev) => !prev)}
            />
            ...
            <SettingsCard title={i18nService.t('keyboardShortcuts')}>
        ...
        return (
          <div className="space-y-4">
            <div className="flex rounded-xl overflow-hidden border dark:border-claude-darkBorder border-claude-border">
              {(['engine', 'memory', 'persona'] as const).map((s) => (
                <button
                  key={s}
                  type="button"
                  onClick={() => setCoworkSection(s)}
                ...
                >
                  {coworkSectionLabels[s]}
                </button>
              ))}
            </div>

            {coworkSection === 'engine' && (
  ...
  return (
    ...
    >
      ...
      >
        {/* Left sidebar */}
        <div className="w-[220px] shrink-0 flex flex-col dark:bg-claude-darkSurfaceMuted bg-claude-surfaceMuted  dark:border-claude-darkBorder border-claude-border rounded-l-2xl overflow-y-auto">
